...
const SAFE_ATTRS = ["id", "full_name", "phone", "email", "role", "is_super", "is_active", "created_at"];
  ...
  try {
    ...
    if (phone?.trim() && phone.trim() !== user.phone) {
      const taken = await User.findOne({
        where: { phone: phone.trim(), id: { [require("sequelize").Op.ne]: user.id } },
      });
      if (taken) return res.status(409).json({ message: "Ce numéro est déjà utilisé." });
