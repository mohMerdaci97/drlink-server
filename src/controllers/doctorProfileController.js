...
const { User, Doctor, Specialty, Appointment, DoctorClinic, Clinic } = require("../models");
...
async function getDoctorWithUser(userId) {
  const doctor = await Doctor.findOne({
    ...
    include: [
      { model: User,      attributes: ["id","full_name","phone","email","created_at"] },
...
exports.getProfile = async (req, res) => {
  try {
    ...
    // Stats summary
    const [total, completed, cancelled, uniquePatients, clinicCount] = await Promise.all([
      Appointment.count({ where: { doctor_id: doctor.id } }),
      Appointment.count({ where: { doctor_id: doctor.id, status: "completed" } }),
      Appointment.count({ where: { doctor_id: doctor.id, status: "cancelled" } }),
      Appointment.count({
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (phone && phone !== user.phone) {
      const exists = await User.findOne({ where: { phone, id: { [Op.ne]: user.id } } });
      if (exists) return res.status(400).json({ message: "Ce numéro est déjà utilisé." });
    }
...
exports.updatePhoto = async (req, res) => {
  try {
    ...
    doctor.photo_url = `/uploads/${req.file.filename}`;
    await doctor.save();
